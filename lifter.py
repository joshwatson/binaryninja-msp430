...
DestOperandsIL = [
    ...
    # ABSOLUTE_MODE
    lambda il, width, reg, value, src: il.store(
        width, il.const_pointer(2, value), src),
...
class Lifter:
    ...
    def lift_type1(il, op, src, dst, flags=None):
        left = SourceOperandsIL[src.mode](
            il, src.width, src.target, src.value
        )

        right = SourceOperandsIL[dst.mode](
            il, dst.width, dst.target, dst.value
        )

        operation = op(
            src.width, left, right, flags=flags
        )
    ...
    def lift_addc(il, instr):
        ...
        addc = il.add(
            instr.src.width, add, il.flag('c'), flags='*'
        )
    ...
    def lift_call(il, instr):
        if instr.src.mode == INDIRECT_AUTOINCREMENT_MODE:
            ...
            temp_expr = il.set_reg(
                2, LLIL_TEMP(0), il.reg(2, instr.src.target)
            )

            call_expr = il.call(il.load(2, il.reg(2, LLIL_TEMP(0))))

            inc_expr = il.set_reg(
                2, instr.src.target, il.add(
                    2,
                    il.reg(2, instr.src.target),
                    il.const(2, 2)
                )
            )
            ...
            call_expr = il.call(
                il.const_pointer(2, instr.src.value)
            )
    ...
    def lift_subc(il, instr):
        ...
        subc = il.sub(
            instr.src.width,
            sub,
            il.not_expr(instr.src.width, il.flag('c')),
            flags='*'
        )
    ...
    def lift_sxt(il, instr):
        src = SourceOperandsIL[instr.src.mode](
            il, 1, instr.src.target, instr.src.value
        )
    ...
    def lift_xor(il, instr):
        xor = Lifter.lift_type1(
            il, il.xor_expr, instr.src, instr.dst, flags='*'
        )
